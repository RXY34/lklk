...
typedef void* (MicrosoftCallingType* PsGetProcessPeb)(
	void* PEProcess
	);
...
= { 0x2f84874d, 0xfd2e, 0x2021, {0x8d, 0x9e, 0x20, 0xa7, 0xaf, 0x9c, 0x32, 0xf1} };
...
static PsGetProcessPeb GetProcessPeb = (PsGetProcessPeb)0;
...
RunCommand(MemoryCommand* cmd)
{
	...
	if (cmd->operation == baseOperation * 0x275)
	{
		...
		GetProcessPeb = (PsGetProcessPeb)cmd->data[3];
		GetProcessWow64Process = (PsGetProcessWow64Process)cmd->data[4];
		ptr64 resultAddr = cmd->data[5];
		*(ptr64*)resultAddr = 1;
		return EFI_SUCCESS;
	}
	...
	if (cmd->operation == baseOperation * 0x666)
	{
		void* pid = (void*)cmd->data[0];
		void* resultAddr = (void*)cmd->data[1];
		void* ProcessPtr = 0;
		...
		*(ptr64*)resultAddr = (ptr64)GetProcessPeb(ProcessPtr); //Return Peb Address
		return EFI_SUCCESS;
	}
